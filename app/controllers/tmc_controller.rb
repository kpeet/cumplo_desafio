   ...
   #@result=Tmc.converter_data(@result)
    ...
    tiposTMC.each do |tipos|
      if(!lista_id_tipos.include? tipos.id_tipo)
        ...
        valores = Tmc.select("valor").where("fecha >= :start_date AND fecha <= :end_date AND tipo= :id_tipo",
        ...
        lista_datos_tipo << [tipos.id_tipo,listaValoresTmc,listaValoresTmc.max, tipos.subtitulo, tipos.titulo]
      end
    end
